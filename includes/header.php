...
<head>
    ...
    <link rel="stylesheet"
        href="<?= count(explode('/', $_SERVER['REQUEST_URI'])) > $count ? '../resources/css/styles.css' : './resources/css/styles.css'; ?>">
...
<body>
    ...
    if ((isset($_POST['email-login']))) {
        ...
        if ($usuario == NULL) {
            ...
            if ($verify == false) {
                echo ("<div>Error al autenticar el usuario</div>");
            } else {
                session_start();                                       
    ...
    <nav class="navbar fixed-top navbar-expand-lg navbar-dark bg-dark">
        <a class="navbar-brand mb-0 h1"
            href="<?= count(explode('/', $_SERVER['REQUEST_URI'])) > $count ? '../index.php' : './index.php'; ?>">Profepedia</a>
        ...
        <div class="collapse navbar-collapse" id="navbarSupportedContent">
            ...
            <ul class="navbar-nav ml-auto">
                <li class="nav-item active">
                    ...
                    <a class="nav-link"
                        href="<?= count(explode('/', $_SERVER['REQUEST_URI'])) > $count ? '../validations/cerrar_sesion.php' : './validations/cerrar_sesion.php'; ?>">Cerrar
                        sesión <span class="sr-only">(current)</span></a>
                    <?php else: ?>
                    <a class="nav-link" href="" data-toggle="modal" data-target="#exampleModal">Iniciar sesión <span
                            ...
                            <div class="card-block">
                                <div class="card-text px-3 pt-3">
                                    <div class="form-group">
                                        <input type="email" class="form-control" name="email-login" value=""
                                            placeholder="Correo" required id="email">
                                    </div>
                                    <div class="form-group">
                                        <input type="password" class="form-control" name="password-login"
                                            placeholder="Contraseña" required value="" id="password">
                                    </div>
                                    <div class="row">
